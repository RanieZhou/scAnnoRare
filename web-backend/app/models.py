...
class Dataset(Base):
    ...
    local_dataset_alias = Column(String(200))
    n_cells = Column(Integer)
    n_genes = Column(Integer)
    obs_columns_json = Column(Text)  # JSON serialized obs list
    var_columns_json = Column(Text)  # JSON serialized var list
    label_distribution_json = Column(Text)  # JSON serialized dict
    rare_candidates_json = Column(Text)  # JSON serialized list
    created_at = Column(Float, default=time.time)
    updated_at = Column(Float, default=time.time)
...
class Experiment(Base):
    ...
    task_type = Column(String(50))  # annotation_evaluation, rare_detection_evaluation
    label_col = Column(String(100))
    batch_col = Column(String(100), nullable=True)
    rare_mode = Column(String(50), nullable=True)  # single_rare, multi_rare_per_class, pooled_rare_vs_nonrare
...
class MethodRun(Base):
    ...
    id = Column(String(50), primary_key=True, index=True)
    experiment_id = Column(String(50), ForeignKey("experiments.id"))
    method_name = Column(String(100), index=True)
    method_type = Column(String(50))  # custom_prediction, imported_prediction, builtin_celltypist...
    input_type = Column(String(50))   # prediction_csv, method_adapter...
    prediction_file_alias = Column(String(200))
    baseline_file_alias = Column(String(200), nullable=True)
...
class Job(Base):
    ...
    id = Column(String(50), primary_key=True, index=True)
    experiment_id = Column(String(50), ForeignKey("experiments.id"))
    method_run_id = Column(String(50), ForeignKey("method_runs.id"))
    agent_id = Column(String(50), ForeignKey("agents.id"))
    job_type = Column(String(50))  # inspect_dataset, evaluate_annotation, evaluate_rare, generate_report
    status = Column(String(20))    # pending, running, success, failed, cancelled
...
class MethodResult(Base):
    ...
    metrics_json = Column(Text)            # overall metrics
    per_class_metrics_json = Column(Text)  # per class precision/recall/f1
...
class Report(Base):
    ...
    method_run_id = Column(String(50), ForeignKey("method_runs.id"))
    job_id = Column(String(50), ForeignKey("jobs.id"))
    report_type = Column(String(20))  # html, md, csv, json
